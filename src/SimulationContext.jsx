...
export const SimulationProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'posts' }, payload => {
        ...
        if (row.parent_id) {
          setPosts(prev => addReplyDeepById(prev, row.parent_id, newPost));
        } else {
          setPosts(prev => {
  ...
  const generateBotText = async (bot, isReply, targetPost = null) => {
    ...
    try {
      ...
      console.log(`[REAL AI GENERATED by ${bot.handle}]:`, generated);
  ...
  useEffect(() => {
    ...
    const infectionInterval = setInterval(() => {
      ...
      setActivePrompts(prev => {
        ...
        console.log(`[INFECTION] ${sourceBot.handle} influenced by ${targetBot.handle}`);
